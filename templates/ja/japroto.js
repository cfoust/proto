...
if (!window.proto) {
	window.proto = {
		...
		render: function(defString, readingString) {
			...
			if (mainReading['kanji'] == '') {
				head.innerHTML = mainReading['kana'];
				stroke.innerHTML = mainReading['kana'];
			} else {
				// Look for the reading string of the main reading
				for (var i = 0; i < readings.length; i++) {
					var parts = readings[i].split('|');
					if (parts[0] == mainReading['kanji'] &&
						parts[1] == mainReading['kana']) {
						head.innerHTML = this.renderFuri(readings[i]);
