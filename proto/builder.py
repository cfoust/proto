...
class Builder:
    """Convenience class for easy generation of decks. Removes the need to worry about csv files being properly named
    and handles all the pathing automatically, so that all the user has to do is provide a valid Anki deck and language
    code and the rest is all handled. """
    ...
    def __init__(self,languageCode,deck):
        """languageCode is the ISO-639-1 language code for the target language. This can actually be any arbitrary
        string that is filesystem compliant, as it is only passed to a path handler.
        deck is a proto.deck.Deck instance."""
    ...
    def _makeTree(self):
        """Internal function that sets up the tree of decks. This lets us store the deck and its bound data all in one
        place and more easily check whether we have everything ready. It isn't actually a tree, just a dictionary, but
        it works similarly."""
    ...
    def deckNeedsData(self, deckName):
        """Returns true if a deck needs data. 'Data' here really means headwords (lemmas). deckName is the Anki-style
        deck name, so something like "German::Nouns" would work."""
